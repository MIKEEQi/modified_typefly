...
class VirtualRobotWrapper(RobotWrapper):
    ...
    def speed_up(self, degree: int) -> Tuple[bool, bool]:
        print(f"->Accelerating to speed {degree}")
        time.sleep(1)
        return True, False

    def drop_goods(self, finish: int) -> Tuple[bool, bool]:
        print(f"-> Dropping goods. Accomplished: {finish}")
        time.sleep(1)
        return True, False

    def carry_goods(self, finish: int) -> Tuple[bool, bool]:
        print(f"-> carrying goods. Accomplished: {finish}")
        time.sleep(1)
        return True, False
